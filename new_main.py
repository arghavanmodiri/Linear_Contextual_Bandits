...
def main_algo(input_dict, mode=None):
    """start the model"""
...
# ========== OLD CODE ==========================
    ...
    save_output_folder = 'saved_output/raw_data' + str(TODAY) + '_' + str(NOW.hour) + str(NOW.minute) + str(
        NOW.second) + "/"
    ...
    for sim in range(0, simulation_count):
        ...
        optimal_action_ratio_per_sim = np.array(
            list((thompson_output[1][i] in thompson_output[0][i]) for i in range(0, user_count))).astype(int)
        ...
        coeff_sign_error_per_sim = np.sign(np.array(true_params_in_hypo)) - np.sign(
            np.array(thompson_output[5])) == np.zeros(len(hypo_params))
        ...
        save_coeff_sign_err_thompson_df = pd.concat([save_coeff_sign_err_thompson_df, coeff_sign_error_per_sim_df],
                                                    axis=1)
        ...
        if (rand_sampling_applied):
            ...
            save_regret_random_df = pd.concat([save_regret_random_df, pd.DataFrame(rand_outputs[2])], ignore_index=True,
                                              axis=1)
        ...
        ols_coeff_dict = {"intercept": regression.params['Intercept'], "d1": regression.params['d1'],
                          "d1*x1": regression.params['d1x1']}
        ...
        ols_optimal_x0 = \
        making_decision.pick_true_optimal_arm(ols_coeff_dict, context_value0, experiment_vars, bandit_arms)[0][0][0]
        # print("OLS optimal action x = 0 is " + str(ols_optimal_x0))

        # Compute optimal arm for X1 = 1
        ols_optimal_x1 = \
        making_decision.pick_true_optimal_arm(ols_coeff_dict, context_value1, experiment_vars, bandit_arms)[0][0][0]
    ...
    if (rand_sampling_applied):
        ...
        suboptimal_ratio_all_policies = np.stack((x0_suboptimal_ratio,
                                                  x1_suboptimal_ratio, x0_suboptimal_ratio_rand,
                                                  x1_suboptimal_ratio_rand))
    else:
    ...
    if (rand_sampling_applied):
        bplots.plot_suboptimal_action_ratio(user_count, ['Thompson Sampling X = 0', 'Thompson Sampling X = 1',
                                                         'Random Sampling X=0', 'Random Sampling X=1'],
